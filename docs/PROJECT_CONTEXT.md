...
- `routeLive` submits ONLY the entry order; SL/TP are stored as virtual levels (no broker bracket). Two app-side watchers enforce them while the backend is up: `PlaybookEntryInvalidationWatcher` cancels a resting *unfilled* entry on zone break, and `PlaybookPositionReconciler` flattens a *filled* `PLAYBOOK_AUTO` position once a fresh live price crosses its virtual SL/TP (`riskdesk.playbook.position-watch.enabled`, default true). Neither survives a backend outage — that residual gap is what real OCO brackets would close (deferred).
- `PlaybookBrokerTruthReconciler` anchors on IBKR truth (not app status): P1 recovers `ENTRY_SUBMITTED` rows whose fill callback was dropped (→ `ACTIVE` so the position-watch protects them) or that are genuinely gone (→ `CANCELLED`); P2 re-cancels an IBKR entry order still working under a row the app marked `CANCELLED`. State-only except P2's bounded cancel; never acts on an `UNAVAILABLE` lookup; fires the R7 reconcile alarm (`riskdesk.playbook.broker-reconcile.enabled`, default true).
