...
public class MentorAnalysisService {
    ...
    private String buildSemanticText(JsonNode payload, MentorStructuredResponse structured) {
        return String.join(" | ",
            ...
            valueOrEmpty(payload.path("market_structure_smc").path("last_event").asText(null)),
            valueOrEmpty(payload.path("momentum_oscillators").path("money_flow_state").asText(null)),
            valueOrEmpty(payload.path("risk_management_gatekeeper").path("reward_to_risk_ratio").asText(null)),
            valueOrEmpty(payload.path("metadata").path("current_price").asText(null)),
            valueOrEmpty(payload.path("macro_correlations_dynamic").path("dxy_trend").asText(null)),
            valueOrEmpty(payload.path("order_flow_and_volume").path("cumulative_delta_trend").asText(null)),
