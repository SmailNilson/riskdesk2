...
const TWEAKS_STYLE = `
  .twk-panel{position:fixed;right:16px;bottom:42px;z-index:2147483646;width:280px;
  ...
  .twk-fab{position:fixed;right:16px;bottom:42px;z-index:2147483645;
...
export function TweaksPanel({ title = 'Tweaks', children }: { title?: string; children: ReactNode }) {
  ...
  const offsetRef = useRef({ x: 16, y: 42 });
  const PAD = 16;
  ...
  return (
    <>
      ...
      {!open && (
        <button type="button" className="twk-fab" aria-label="Open tweaks" title="Tweaks" onClick={() => setOpen(true)}>
          ✎
        </button>
      )}
      {open && (
